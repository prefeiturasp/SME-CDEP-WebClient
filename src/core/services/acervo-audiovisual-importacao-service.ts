...
const obterImportacaoPendente = () =>
  obterRegistro<ImportacaoArquivoRetornoDTO>(URL_API_ACERVO_AUDIOVISUAL_IMPORTACAO_PLANILHA);

const removerLinhaDoArquivo = (id: number, numeroLinha: number) =>
  alterarRegistroParcial(`${URL_API_ACERVO_AUDIOVISUAL_IMPORTACAO_PLANILHA}/${id}`, {
    numeroLinha,
  });

const atualizarLinhaParaSucesso = (id: number, numeroLinha: number) =>
  alterarRegistroParcial(
    `${URL_API_ACERVO_AUDIOVISUAL_IMPORTACAO_PLANILHA}/atualizar-linha/${id}/sucesso`,
    { numeroLinha },
  );
