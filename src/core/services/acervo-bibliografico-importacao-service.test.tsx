import service from './acervo-bibliografico-importacao-service';
...
describe('acervo bibliografico importacao service', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('removerLinhaDoArquivo chama alterarRegistroParcial corretamente', async () => {
    ...
    expect(alterarRegistroParcial).toHaveBeenCalledWith(
      `${URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA}/1`,
      { numeroLinha: 5 },
    );
  });
  ...
  it('atualizarLinhaParaSucesso chama alterarRegistroParcial corretamente', async () => {
    ...
    expect(alterarRegistroParcial).toHaveBeenCalledTimes(1);
    expect(alterarRegistroParcial).toHaveBeenCalledWith(
      `${URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA}/atualizar-linha/2/sucesso`,
      { numeroLinha: 7 },
    );
  });
});