import { URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA } from '../constants/urls-api';
...
const importarArquivo = (file: File) => {
  ...
  return inserirRegistro<ImportacaoArquivoRetornoDTO>(
    URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA,
    fmData,
    config,
  );
};
...
const obterImportacaoPendente = () =>
  obterRegistro<ImportacaoArquivoRetornoDTO>(URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA);

const removerLinhaDoArquivo = (id: number, numeroLinha: number) =>
  alterarRegistroParcial(`${URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA}/${id}`, {
    numeroLinha,
  });

const atualizarLinhaParaSucesso = (id: number, numeroLinha: number) =>
  alterarRegistroParcial(
    `${URL_API_ACERVO_BIBLIOGRAFICO_IMPORTACAO_PLANILHA}/atualizar-linha/${id}/sucesso`,
    { numeroLinha },
  );
