import Redefinir_Senha_CDEP_Localizadores from '../locators/redefinir_senha_locators'

const redefinir_senha_CDEP_Localizadores = new Redefinir_Senha_CDEP_Localizadores()

Cypress.Commands.add('clicar_botao_esqueci_senha', () => {
	cy.get(redefinir_senha_CDEP_Localizadores.botao_esqueci_senha())
	  .should('be.visible').click()	
})

Cypress.Commands.add('clicar_botao_continuar_esqueci_senha', () => {
  cy.get(redefinir_senha_CDEP_Localizadores.input_usuario_esqueci_senha())
    .should('be.visible').type(Cypress.env('LOGIN_ADM_GERAL'))

	cy.get(redefinir_senha_CDEP_Localizadores.botao_continuar_esqueci_senha())
	  .should('be.visible').click()	
})

Cypress.Commands.add('validar_enviado_esqueci_senha', () => {
  cy.get(redefinir_senha_CDEP_Localizadores.modal_email_enviado())
    .should('be.visible')
...
Cypress.Commands.add('validar_enviado_esqueci_senha_invalido', () => {
  cy.get(redefinir_senha_CDEP_Localizadores.alerta_usuario_invalido(), { timeout: 60000 })
    .should('be.visible')
    .should('contain.text', 'Login não encontrado')
})
...
Cypress.Commands.add('validar_enviado_esqueci_senha_caracteres', () => {
  cy.get(redefinir_senha_CDEP_Localizadores.alerta_caracteres(), { timeout: 60000 })
    .should('be.visible')
